...
class RBTreeNodeTraitsTemplate {
    ...
    /**
     * set the \c RBNodePtr \c n to the given \c RBNodePtr \c nNew. Note that if color, nullness and threading properties are stored in the \c RBNodePtr structure, then these must be preserved by the set operation. This will generally just amount to n = nNew.
     */
...
public:
     ...
     * @param rbnt an \c RBTreeNodeTraits object
     */
    static void insert(RBNodePtr& h, RBNodePtr n, const RBKey& key, const RBNT& rbnt) {
        ...
        while (true) {
            stack.push(hPtr);
            if (!rbnt.isNull(rbnt.left(*hPtr)) && !rbnt.isNull(rbnt.right(*hPtr)) && rbnt.getColor(rbnt.left(*hPtr))
                    && rbnt.getColor(rbnt.right(*hPtr))) split(*hPtr, rbnt);
            if (rbnt.less(key, *hPtr)) if (rbnt.isNull(rbnt.left(*hPtr))) {
                addNodeLeft(*hPtr, n, rbnt);
                break;
            } else hPtr = &(rbnt.left(*hPtr));
            else if (rbnt.isNull(rbnt.right(*hPtr))) {
                addNodeRight(*hPtr, n, rbnt);
                break;
            } else hPtr = &(rbnt.right(*hPtr));
        }
        ...
        while (!stack.empty()) {
            ...
            if (!rbnt.isNull(rbnt.right(*hPtr)) && rbnt.getColor(rbnt.right(*hPtr))
                    && (rbnt.isNull(rbnt.left(*hPtr)) || !rbnt.getColor(rbnt.left(*hPtr)))) rotateLeft(*hPtr, rbnt);
            if (!rbnt.isNull(rbnt.left(*hPtr)) && rbnt.getColor(rbnt.left(*hPtr))
                    && !rbnt.isNull(rbnt.left(rbnt.left(*hPtr))) && rbnt.getColor(rbnt.left(rbnt.left(*hPtr)))) rotateRight(
                    *hPtr, rbnt);
        }
     ...
     * @param rbnt an \c RBTreeNodeTraits object
     */
    static void fixThreading(const RBNodePtr& n, const RBNT& rbnt) {
        RBNodePtr* xPtr = &(rbnt.left(n));
        if (!rbnt.isNull(*xPtr)) {
            while (!rbnt.isNull(*xPtr))
                xPtr = &(rbnt.right(*xPtr));
            rbnt.set(*xPtr, n);
            rbnt.setThread(*xPtr);
        }
        xPtr = &(rbnt.right(n));
...
private:
    static void addNodeLeft(RBNodePtr& h, RBNodePtr& n, const RBNT& rbnt) {
    ...
    static void addNodeRight(RBNodePtr& h, RBNodePtr& n, const RBNT& rbnt) {
    ...
    static void rotateLeft(RBNodePtr& h, const RBNT& rbnt) {
        RBNodePtr x;
    ...
    static void rotateRight(RBNodePtr& h, const RBNT& rbnt) {
        RBNodePtr x;
    ...
    static void split(RBNodePtr& h, const RBNT& rbnt) {
        rbnt.setColor(h, true);
