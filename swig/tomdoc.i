...
`regularization(vPC=-1, vZ=-1, vMin=-1, preset=\"\") -> tuple`  
...
----------
* `vPC` :  
    number of pseudo-counts to add before computing the variance  
* `vZ` :  
    the size of the confidence interval used in the variance computation,
    measured by z-score  
* `vMin` :  
...
`Oom(dimension, nOutputSymbols, nInputSymbols=0, randomExponent=0,
    zero_threshold=0, randomSource=Random())`  
...
`wordsFromData(dataSuffixTree, minLength=0, maxLength=0, minRelevance=1.0,
    maxWords=0, uniquePositions=false, relevance=stree::PositionRelevance()) ->
...
* `relevance` :  
    ...
    inherited from `PositionRelevance` with an overwritten `.compute()` method.  
